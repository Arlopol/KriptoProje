    ...
    def next(self):
        ...
        if self.position:
            if pl_pct < -self.stop_loss_pct:
                closed_reason = f"Stop Loss (Base %{self.stop_loss_pct*100:.0f} Zarar) Tetiklendi. Pozisyon Kapatılıyor."
        
        # --- LONG SİNYALİ ---
        # Threshold'ları class attribute'dan al
        THRESHOLD_BUY = self.buy_threshold
        THRESHOLD_SELL = self.sell_threshold
        
        # BOĞA PİYASASINDA ALIM EŞİĞİNİ DÜŞÜR
        if is_bull:
            THRESHOLD_BUY = 0.50
        ...
        elif prob > THRESHOLD_BUY: # 0.60 veya 0.50
            if is_short:
